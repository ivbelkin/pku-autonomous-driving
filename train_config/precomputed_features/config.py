...
train_ds = D.PKUSingleObjectWithPrecomputedImageFeaturesDataset(
    extract_featues_fn=D.ResnetFeatureExtractor(model=models.resnet101(pretrained=True).float().cuda(), dct_key='image'), 
    workdir=workdir, 
    file_name='features', 
    dct_key='image_featues',

...
workdir = os.path.join(cfg.WORKDIR, 'model_1', 'stage_2_bbox_aug_2')
...
model.load_state_dict(torch.load(os.path.join(os.path.join(cfg.WORKDIR, 'model_1', 'stage_2_bbox_aug'), 'checkpoints', '_model_10.pth')))
# for p in model.backbone.parameters():
#     p.requires_grad = False

optimizer = torch.optim.Adam(model.parameters(), lr=0.0001)
optimizer.load_state_dict(torch.load(os.path.join(os.path.join(cfg.WORKDIR, 'model_1', 'stage_2_bbox_aug'), 'checkpoints', '_optimizer_10.pth')))

# scheduler = PiecewiseLinear(optimizer, "lr",
#                             milestones_values=[(0, 0.000001), (iters_per_epoch, 0.0001), (6 * iters_per_epoch, 0.000001), (n_epochs * iters_per_epoch, 0.0000001)])
scheduler = LRScheduler(lr_scheduler=StepLR(optimizer=optimizer, step_size=iters_per_epoch, gamma=0.8))
